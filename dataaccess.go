...
var queries map[string]*sqlQueries = map[string]*sqlQueries{
	"mysql": &sqlQueries{
		...
		PoolInfo:              "SELECT p.name, sum(m.volbytes) AS bytes, count(*) AS volumes, '0' AS prunable, COALESCE(TIMESTAMPADD(SECOND, m.volretention, m.lastwritten) < NOW(), false) AS expired FROM Media m LEFT JOIN Pool p ON m.poolid = p.poolid GROUP BY p.name, prunable, expired",
